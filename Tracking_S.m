...
for k = 1:Nscans
    ...
    [Ind(k), th(1:Nsamp_to_measure, k)] = CFAR(path103(1:Nsamp_to_measure, k), 21, 0.1, 16);
    est_rng(k) = rng(Ind(k));
end
...
%     r4 = med104(i);
    ...
    Theta_x(i) = Theta(1);
    Theta_y(i) = Theta(2);
    r_est(i) = sqrt(Theta_x(i).^2 + Theta_y(i).^2);
    
%     U_x(i) = H*cov(Theta_x, Theta_x)*H';
%     U_y(i) = H*cov(Theta_y, Theta_y)*H';
