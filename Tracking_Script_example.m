...
plot(P102(1),P103(2),'ok','markerfacecolor','k');text(P102(1)+.1,P102(2),'103')
plot(P104(1),P102(2),'ok','markerfacecolor','k');text(P104(1)+.1,P104(2),'104')
...
% 104
figure;imagesc(abs(data103));title('104')
...
rngSph104 = rng(345); % range of sphere in unit 104 (m)
...
data103_algnd = data103(:,t102start_i:t102stop_i);
...
%% Background removal
close all;
BGidx = 310:350;  % select the slow time indexes containing the background data
...
for i1=1:Nscans % Long time
    ...
    while abs(path101(i2,i1)) < 0.02 && i2 < Nrng-1    % threshold 0.06
        i2 = i2 + 1;
    end
...
for i1=1:Nscans % Long time
    ...
    while abs(path102(i2,i1)) < 0.03&& i2 < Nrng-2
        i2 = i2 + 1;
    end
    estimated_range(i1) = rng(i2);
end
...
for i1=1:Nscans % Long time
    i2 = 1; % Avoiding picking antenna coupling
    while abs(path103(i2,i1)) < 0.03 && i2 < Nrng-1
        i2 = i2 + 1;
    end
    estimated_range(i1) = rng(i2);
end
...
figure;plot(x14(:,1),y14(:,1));title('101 - 104');axis([-1.5 1.5 -1.5 1.5])  
...
figure;plot(x24(:,2),y24(:,1));title('102 - 104');axis([-1.5 1.5 -1.5 1.5])  
...
figure;plot(x34(:,2),y34(:,2));title('102 - 104');axis([-1.5 1.5 -1.5 1.5])  
...
% 
x = nanmean([x14(:,2), x12(:,1)],2);
y = nanmean([y14(:,2), x12(:,1)],2);
